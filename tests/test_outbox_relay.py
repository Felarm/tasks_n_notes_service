...
from events.outbox.outbox_kafka_relay import process_outbox_messages


@kafka_broker.subscriber(settings.TASKS_TOPIC_NAME)
async def tasks_subscriber() -> None:
    print("Yo from tasks_subscriber")


@kafka_broker.subscriber(settings.NOTES_TOPIC_NAME)
...
async def test_process_outbox_messages(outbox_repo, outbox_messages):
    async with TestKafkaBroker(kafka_broker) as test_broker:
